# evaluate.py
# Run this after main.py has generated Q.npy and rewards.npy.
# Compares Q-learning against four baseline policies and plots results.
#
# Usage:
#   python main.py       <- trains Q-learning, saves Q.npy + rewards.npy
#   python evaluate.py   <- loads results and produces all charts
...
results = {
    'Q-learning':     evaluate(lambda s: int(np.argmax(Q[s]))),
...
plt.legend()
plt.tight_layout()
plt.savefig('plot_learning_curve.png', dpi=150)
plt.show()
...
plt.tight_layout()
plt.savefig('plot_reward_comparison.png', dpi=150)
plt.show()
print('Saved: plot_reward_comparison.png')
...
plt.tight_layout()
plt.savefig('plot_stockout_holding.png', dpi=150)
plt.show()
print('Saved: plot_stockout_holding.png')
...
fig.suptitle('Learned policy (Q-learning): best action per state', y=1.02)
plt.colorbar(im, ax=axes[-1], ticks=[0,1,2,3],
             label='Action (0=none 1=small 2=med 3=large)')
plt.tight_layout()
plt.savefig('plot_policy_heatmap.png', dpi=150, bbox_inches='tight')
plt.show()
